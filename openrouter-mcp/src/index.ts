...
const pythonScriptPath = "C:\\Users\\Joris\\Documents\\Cline\\MCP\\python-ai-tools\\call_openrouter.py";
...
const server = new Server(
  {
    name: "openrouter-mcp",
    version: "0.1.0",
    description: "MCP Server to call Deepseek via OpenRouter using a Python script."
  },
  {
    capabilities: {
      tools: {},
 ...
 * Exposes a single "call_deepseek" tool.
 */
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: "call_deepseek",
        description: "Calls the Deepseek model via OpenRouter using a Python script.",
        inputSchema: {
          type: "object",
          properties: {
            prompt: {
              type: "string",
              description: "The prompt to send to the Deepseek model."
            }
          },
          required: ["prompt"]
        }
...
/**
 * Handler for the call_deepseek tool.
 * Executes the Python script with the provided prompt.
 */
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name !== "call_deepseek") {
  ...
  try {
    ...
    // The OPENROUTER_API_KEY is expected to be set in the environment where this MCP server runs (via cline_mcp_settings.json)
...
async function main() {
  ...
  console.error('OpenRouter MCP server running on stdio'); // Log to stderr so it doesn't interfere with stdout communication
}
